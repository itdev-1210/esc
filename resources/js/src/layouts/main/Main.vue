...
export default {
  ...
  watch: {
    ...
    isThemeDark(val) {
      const color = this.navbarColor == "#fff" && val ? "#3490dc" : "#fff"
  ...
  methods: {
    ...
    updateNavbar(val) {
      if (val == "static") this.updateNavbarColor(this.isThemeDark ? "#3490dc" : "#fff")
      this.navbarType = val
    },
  ...
  created() {
    const color = this.navbarColor == "#fff" && this.isThemeDark ? "#3490dc" : this.navbarColor
    this.updateNavbarColor(color)
